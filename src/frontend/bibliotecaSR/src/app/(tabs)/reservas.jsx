...
export default function Reservas() {
  ...
  const fetchReservas = async () => {
    try {
      const response = await api.get(`/usuarios/${user.id}/reservas`);
      const data = response.data;

      // abertas: em análise (0) ou aguardando retirada (2)
      setAtivas(data.filter((r) => r.status === 0 || r.status === 2));
      // histórico: o resto
      setHistorico(data.filter((r) => r.status !== 0 && r.status !== 2));
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {ativas.length > 0 ? (
          ativas.map((item) => <ReservaCard key={item.id} item={item} />)
        ) : (
        ...
        {historico.length > 0 ? (
          historico.map((item) => <ReservaCard key={item.id} item={item} />)
        ) : (
